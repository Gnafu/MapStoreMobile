...
/**
 * A fragment representing a single Pending Mission detail screen. This fragment is either contained in a {@link PendingMissionListActivity} in
 * two-pane mode (on tablets) or a {@link PendingMissionDetailActivity} on handsets.
 */
public class PendingMissionDetailFragment extends MapFragment implements LoaderCallbacks<Void> {
    ...
    /**
     * The <LinearLayout> inside the <ScrollView> This displays dynamic content as the other forms.
     */
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        Log.d(TAG, "onCreateView(): container = " + container + "savedInstanceState = "
                + savedInstanceState);

        if (mScrollView == null) {
            // normally inflate the view hierarchy
            mScrollView = (ScrollView) inflater.inflate(R.layout.preview_page_fragment, container,
                    false);
        ...
        if (id == R.id.accept) {
            ...
            if (geoPoint != null) {
                ...
                if (updatedPoint != null) {
                    mapIntent.putExtra(SimpleMapActivity.ARG_SECOND_POINT_LAT,
                            updatedPoint.latitude);
                    mapIntent.putExtra(SimpleMapActivity.ARG_SECOND_POINT_LON,
             ...
             * possibilities :
             * 
             * Uri.parse("google.navigation:q= lat,lon) //won't start navigation Uri.parse("geo:latitude,longitude //will only zoom map on position
    ...
    /**
     * reads out the missions origin GeoPoint by retrieving the mapView field and using it to extract it out of the missions tags
     * 
    ...
    public GeoPoint getUpdatedGeoPoint() {
        ...
        final String s = "SELECT Y(" + f.fieldId + "), X(" + f.fieldId + ") FROM '" + tableName
                + "' WHERE ORIGIN_ID = '" + originIDString + "';";
    ...
    private void buildForm() {
        ...
        if (mDone)
            return;
        MissionTemplate t = MissionUtils.getDefaultTemplate(getSherlockActivity());
        FormBuilder.buildForm(getActivity(), this.mFormView, t.preview.fields, mission);// TODO page is not enough, some data should be accessible
                                                                                        // like constants and data
